...
class MapOrmResultSet(private val data: OrmResult<*>) : OrmResultSet {
    ...
    override fun get(column: Column<*, Instant>): Instant = instant(column.name)!!
    ...
    override fun nullable(column: Column<*, Instant?>): Instant? = instant(column.name)

    private fun instant(name: String) = when (val value = data[name]) {
