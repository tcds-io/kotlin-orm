...
class NullableIntegerParamTest {
    ...
    fun `given a param when bind is invoked then bind its value into the statement`() {
        val param = NullableIntegerParam("count", value)
    ...
    fun `given null value when bind is invoked then bind its value into the statement`() {
        val param = NullableIntegerParam("count", null)

        param.bind(stmt, 1)

        verify(exactly = 1) { stmt.setNull(1, Types.INTEGER) }
    }
